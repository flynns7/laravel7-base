...
@section('content')
    ...
    <!-- Modal -->
    <div class="modal fade" id="modal-icon" tabindex="-1" role="dialog" aria-labelledby="modal-iconLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modal-iconLabel">Daftar Icon</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body" id="content-list-icon" style="max-height: 400px; overflow-x: auto;">
...
@section('js')
    <script>
        ...
        const listIcon = JSON.parse(listFA)
        let datatable;
        $('document').ready(function() {
            ...
            $('#datatable').setOnActionClickListener(function(type, data) {
                switch (type) {
                    case 'edit':
                            ...
                            onContentReady: function() {
                                ...
                                roles.forEach(function(item) {
                                    if (data.role_ids != null) {
                                        console.log(`,${item.id},`.split(",").includes(data.role_ids))
                                        if (data.role_ids.split(",").includes(`${item.id}`))
            ...
            $("input[name=icon]").on("click, focus", function() {
        ...
        function setIconValue(element) {
            ...
            $("input[name=icon]").val(iconClassName)
            $("#modal-icon").modal('hide')
        }

        function searchIcon(element) {
            const value = element.value
            const filteredIcon = listIcon.filter((item) => {
                return (item.name.toLowerCase().includes(value.toLowerCase())) ? true : false
            })
            renderTable(filteredIcon)
        }
        
        function loadListIcon() {
            const container = $("#content-list-icon")
            let contentTable = ``;
            listIcon.forEach(element => {
                contentTable +=
                    `<tr style="cursor:pointer;" onclick="setIconValue(this)"><td>${element.icon}</td><td>${element.name}</td></tr>`
            });
            const table = `<table class="table table-sm table-bordered" id="table">${contentTable}</table>`
            container.append(
                `<input type="text" class="form-control form-control-sm mb-3" placeholder="Cari Icon" onKeyup="searchIcon(this)">`
                )
